...
RegisterNetEvent('rw-contacts:getDialogue', function(data)
    if PlayerData then
    ...
    TriggerEvent("rw-contacts:showMenu", data.n)
    SetNuiFocus(true, true)
end)

RegisterNetEvent("rw-contacts:showMenu", function(npc)
    local domain = npc.domain
    currentDomain = domain
    
    if Framework == 'qbcore' then
        QBCore.Functions.TriggerCallback('rw-contacts:getRep', function(result)
            SendNUIMessage({
                type = "open",
                ui = 1,
                options = npc.options,
    ...
    elseif Framework == 'esx' then
        ESX.TriggerServerCallback('rw-contacts:getRep', function(result)
            SendNUIMessage({
                type = "open",
                ui = 1,
                options = npc.options,
...
RegisterNetEvent("rw-contacts:showTablet", function()
    if Framework == 'qbcore' then
        QBCore.Functions.TriggerCallback('rw-contacts:getAllReps', function(result)
    ...
    elseif Framework == 'esx' then
        ESX.TriggerServerCallback('rw-contacts:getAllReps', function(result)
            local final = {}
...
RegisterNUICallback("rw-contacts:hideMenu", function()
    SetNuiFocus(false, false)
    DestroyCamera()
end)

RegisterNUICallback("rw-contacts:setMark", function(data)
    local x = tonumber(data.x)
    local y = tonumber(data.y)
    
    if x and y then
        if Framework == 'qbcore' then
            QBCore.Functions.Notify("Marked the contact on GPS", 'error')
        elseif Framework == 'esx' then
...
RegisterNUICallback("buyItem", function(data)
    TriggerServerEvent("rw-contacts:payItem", data)
end)
...
RegisterNUICallback("rw-contacts:exe", function(data)
    local cRep = getRep()
        ...
        options = {
            {
                ...
                action = function(entity, coords, args)
                    TriggerEvent("rw-contacts:getDialogue", args)
                end,
                args = {
                    n = cfg,
                }
...
Citizen.CreateThread(function()
    ...
    exports["rw-contacts"]:createContact(cfg, "silavtora", "Talk")
end)  ]]