...
RegisterNetEvent('rw-contacts:modifyRep')
AddEventHandler('rw-contacts:modifyRep', function(domain, reputationChange)
    local src = source
    local player = nil
...
RegisterNetEvent('rw-contacts:modifyRepS')
AddEventHandler('rw-contacts:modifyRepS', function(id, domain, reputationChange)
    modifyReputation(id, domain, reputationChange)
end)
exports('modifyReputation', modifyReputation)


RegisterNetEvent('rw-contacts:payItem')
AddEventHandler('rw-contacts:payItem', function(data)
    local player = nil
    local src = source
...
if Framework == 'qbcore' then
    QBCore.Functions.CreateCallback('rw-contacts:getRep', function(source, cb, domain)
    ...
    QBCore.Functions.CreateCallback('rw-contacts:getAllReps', function(source, cb)
        local player = QBCore.Functions.GetPlayer(source)
        if player then
...
elseif Framework == 'esx' then
    ESX.RegisterServerCallback('rw-contacts:getRep', function(source, cb, domain)
    ...
    ESX.RegisterServerCallback('rw-contacts:getAllReps', function(source, cb)
        local player = ESX.GetPlayerFromId(source)
        if player then
...
-- you can use it to reward reputation from other scripts
RegisterNetEvent('reputation:server:reward', function(amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.update([[
        INSERT INTO reputation (citizen_id, domain, reputation)
        VALUES (?, ?, ?)
        ON DUPLICATE KEY UPDATE reputation = reputation + VALUES(reputation)
    ]], {
