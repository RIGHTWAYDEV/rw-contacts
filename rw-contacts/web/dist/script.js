    ...
    document
      ...
      .addEventListener("click", () => {
        if (option.data) {
          ...
          $.post(`https://${GetParentResourceName()}/rw-contacts:exe`, d);
          return;
        ...
        $.post(
          `https://${GetParentResourceName()}/rw-contacts:exe`,
          JSON.stringify({
            event: option.event,
            args: option.args,
            type: option.type,
        ...
        body.fadeOut(700, function () {
          $.post(`https://${GetParentResourceName()}/rw-contacts:hideMenu`);
    ...
    if (button) {
      ...
      button.onclick = (function (option) {
        return function () {
          ...
          body.fadeOut(700, function () {
            $.post(
              `https://${GetParentResourceName()}/rw-contacts:hideMenu`
            );
            document.getElementById("extraContent").innerHTML = "";
          });
        };
      ...
      event.data.final.forEach((npc, index) => {
        ...
        if (markButton) {
          markButton.addEventListener("click", function () {
            ...
            $.post(
              `https://${GetParentResourceName()}/rw-contacts:setMark`,
              JSON.stringify({ x, y })
            );
